    ...
    <div :data-node-id="卡片数据.id">
      ...
      >
        ...
        >
          ...
          <div :style="`font-size:xx-small;color:${卡片数据.attrs.color}`">双击标记展开,拖拽移动</div>
        </div>
      </el-popover>
      <div v-if="!卡片数据.attrs.folded" @dblclick="卡片数据.attrs.folded = !卡片数据.attrs.folded">
        <div
          :style="`position:absolute;
                                  overflow:hidden;
        ...
        >
          <div class="cc-node-toolbar">
            ...
            <span class="el-icon-focus" @click="$事件总线.$emit('定位至卡片', 卡片数据.id)"></span>
...
module.exports = {
  ...
  methods: {
    ...
    计算可见性: async function () {
      ...
      if (
        window.pageYOffset > (卡片数据.attrs.top + 卡片数据.attrs.height)*$当前窗口状态.缩放倍数 ||
        window.pageYOffset + window.innerHeight < (卡片数据.attrs.top)*$当前窗口状态.缩放倍数 ||
        window.pageXOffset > (卡片数据.attrs.left + 卡片数据.attrs.width)*$当前窗口状态.缩放倍数 ||
        window.pageXOffset + window.innerWidth < (卡片数据.attrs.left)*$当前窗口状态.缩放倍数
      ) {
        // 不可见标记数组.push(标记);
        this.hide = true;
      }
